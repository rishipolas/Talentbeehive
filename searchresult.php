...
<body>
  ...
  <form action="home-search-result.php" method="post" class=" form-inline .nav-form-search" style="margin: 0 auto;">
                                 ...
                                 <div class="form-group">
                                      <input type="text" id="idlocation" list="listlocation" name="location" class="form-control" placeholder="location">
			<datalist id="idlocation">  
<?php
$sql2 = mysqli_query($con, "SELECT * From location");
$ro2 = mysqli_num_rows($sql2);
while ($ro2 = mysqli_fetch_array($sql2)){
?>
     <option value="<?php echo $ro2['city'] ?>" >
...
?>
                                 ...
                                 <div class="form-group">
                                   <input type="text" id="idrole" list="listrole" class="form-control" name="role" placeholder="Functional Role">  
 <datalist id="idrole">  
 <?php
...
?>
 ...
 <?php
	...
	$location = " AND location='".mysqli_real_escape_string($con,$_REQUEST["location"])."'";	
}
...
if ($_REQUEST["role"]<>'' and $_REQUEST["location"]<>'') {
	$q = "SELECT atype,openings,type,qualification,title,location,role,skills,description,date,functionality.name as 'tname',functionality.skills  FROM functionality WHERE role >= '".mysqli_real_escape_string($con,$_REQUEST["role"])."' AND location <= '".($_REQUEST["location"])."' ".$role.$location;
}  else if ($_REQUEST["role"]<>'') {
	$q = "SELECT atype,openings,type,qualification,title,location,role,skills,description,date,functionality.name as 'tname',functionality.skills  FROM functionality WHERE role >= '".mysqli_real_escape_string($con,$_REQUEST["role"])."'".$role;
} else if ($_REQUEST["location"]<>'') {
	$q = "SELECT atype,openings,type,qualification,title,location,role,skills,description,date,functionality.name as 'tname',functionality.skills  FROM functionality WHERE location <= '".mysqli_real_escape_string($con,$_REQUEST["location"])."'".$location;
}else if(empty($_REQUEST["location"]) and empty($_REQUEST["role"])) {
	
	$q = "SELECT * FROM functionality where location>0 and role>0 ";
