...
$query = $DB->query("SELECT email FROM institute_registration where id=".$instid."");
$getemail =$query->fetch_assoc();

$query1 = $DB->query("SELECT cname FROM users where id=".$cid."");
...
try {
  ...
  $mail->isHTML(true);                                  // Set email format to HTML
	$mail->Subject   = 'Shortlisted candidates for ' .$getname['cname']. ' ';
...
<div style="margin:40px;">
              ...
              <p>Thank you for using campus hiring feature of talentbeehive.com.<br>
               Attatched is the list of candidates shortlisted by ' .$getname['cname']. '. <br><p>
              <br>
...
This is to acknowledge that you have registered as a Institute with TalentBeehive, have agreed to abide to the terms of use and ourprivacy commitment,<br></div>
...
echo ("<SCRIPT LANGUAGE='JavaScript'>			          
	         		window.location.href='shortlist_company_mail.php?instid=".$instid."&& cid=".$cid."';
	                        </SCRIPT>");
  }
 catch (Exception $e) { 
 echo ("<SCRIPT LANGUAGE='JavaScript'>
